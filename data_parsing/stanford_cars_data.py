...
class ExtractedFeatureDataset(Dataset):
    def __init__(self, name, data, dataset_orig):
        ...
        if hasattr(dataset_orig, "targets"):
            self.targets = dataset_orig.targets
        else:
            self.targets = []
            print("Generating model targets")
            targets_f = "./out/stanfordcars_targets.pkl"
            if os.path.exists(targets_f):
                with open(targets_f, "rb") as pickle_targets:
                    self.targets = pickle.load(pickle_targets)
            else:
                self.targets = [v[1] for v in tqdm(dataset_orig)]
                with open(targets_f, "wb") as pickle_targets:
                    pickle.dump(self.targets, pickle_targets)
...
class StanfordCarsDataset():
    def __init__(self, path):
        self.dataset_orig = {x: datasets.StanfordCars(path, split=x, download=True, transform=transforms[x]) for x in phases}
    ...
    def convert_features(self, file_path, device, feature_extractor=None, BS=32):
        ...
        for phase in ['train', 'test']:
            ...
            self.dataset[phase] = ExtractedFeatureDataset("stanfordcars", [], self.dataset_orig[phase])
    ...
    def load_features(self, file_path):
        with open(file_path, "rb") as pickle_f:
            ...
            self.dataset['train'] = ExtractedFeatureDataset("stanfordcars", load_pkl['train'], self.dataset_orig['train'])
            self.dataset['test'] = ExtractedFeatureDataset("stanfordcars", load_pkl['test'], self.dataset_orig['test'])

if __name__=="__main__":
    dset = StanfordCarsDataset("./out/data/stanfordcars")
    ...
    dset.convert_features("./out/stanfordcarsresdensefeatures.pkl", device, f_extractor)
