...
from models.models import ServerMLPTwoHidden, DeviceMLPTwoHidden
...
GPU = 3 # Change depending on others' use of GPUS
dname = args.dataset
...
early_cutoff = 0.005
out_dir = f"istproxtwolayer_{dname}_res"
...
def aggregate(w_glob, w, fc1_index, fc2_index):
    ...
    for i in range(len(w)):
        ...
        layer_bn2_weight.append(w[i]['bn2.weight'].cpu())
        layer_bn2_bias.append(w[i]['bn2.bias'].cpu())
        layer_fc3_weight.append(w[i]['fc3.weight'].cpu())
    ...
    update_tensor_by_update_lists_dim_01(new_server_glob['fc2.weight'], layer_fc2_weight,
                                         fc2_index, fc1_index)
    update_tensor_by_update_lists_dim_0(new_server_glob['bn2.weight'], layer_bn2_weight, fc2_index)
    update_tensor_by_update_lists_dim_0(new_server_glob['bn2.bias'], layer_bn2_bias, fc2_index)
    update_tensor_by_update_lists_dim_1(new_server_glob['fc3.weight'], layer_fc3_weight, fc2_index)
...
def train_model_federated(num_users, part_ratio, device, model, local_model, feature_extractor, criterion, mu, diric, learning_rate, num_epochs=25, local_epochs=1):
    ...
    for epoch in range(num_epochs):
        ...
        # Used in IST
        hidden1, hidden2 = local_model.get_hidden_dim()
        index_hidden_layer1, index_hidden_layer2 = model.partition(part_users, hidden1, hidden2)
        fc1_weight_partition = partition_FC_layer_by_output_dim_0(w_glob['fc1.weight'], index_hidden_layer1)
        bn1_weight_partition, bn1_bias_partition = partition_BN_layer(w_glob['bn1.weight'], w_glob['bn1.bias'], index_hidden_layer1)
        fc2_weight_partition = partition_FC_layer_by_dim_01(w_glob['fc2.weight'], index_hidden_layer2, index_hidden_layer1)
        bn2_weight_partition, bn2_bias_partition = partition_BN_layer(w_glob['bn2.weight'], w_glob['bn2.bias'], index_hidden_layer2)
        fc3_weight_partition = partition_FC_layer_by_input_dim_1(w_glob['fc3.weight'], index_hidden_layer2)
        ...
        for u_id, user in enumerate(idxs_users):
            ...
            w_local['bn2.weight'] = bn2_weight_partition[u_id]
            w_local['bn2.bias'] = bn2_bias_partition[u_id]    
            w_local['fc3.weight'] = fc3_weight_partition[u_id]
        ...
        w_new = aggregate(w_glob, local_weights, index_hidden_layer1, index_hidden_layer2)
    ...
    tracker.write(f"{out_dir}/{dataset.dataset['train'].name}_fedistproxtwolayer_dir{diric}_user{num_users}_lr{learning_rate}_mu{mu}_frac{part_ratio}_localit{local_epochs}")
...
for diric in DIRIC:
    for lr in LR:
        for p in parameters:
            ...
            participating_users= max(1, int(USERS * frac)) # Minimum one user
            local_hidden_size1 = hidden_size1//participating_users
            local_hidden_size2 = hidden_size2//participating_users

            model = ServerMLPTwoHidden(3072, hidden_size1, hidden_size2, OUTPUT_SIZE)
            local_model = DeviceMLPTwoHidden(3072, local_hidden_size1, local_hidden_size2, OUTPUT_SIZE)
