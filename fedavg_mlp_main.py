...
early_cutoff = 0.005
out_dir = f"fedavg_{hidden_size}_{dname}_res"
...
def train_model_federated(num_users, part_ratio, device, model, local_model, feature_extractor, criterion, diric, learning_rate, num_epochs=25, local_epochs=1):
    ...
    tracker.write(f"{out_dir}/{dataset.dataset['train'].name}_fedavg_dir{diric}_user{num_users}_lr{lr}_frac{part_ratio}_localit{local_epochs}")
