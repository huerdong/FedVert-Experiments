...
parameters = {
    1000: [
        ...
        {"frac": 0.02,"loc": 5,"mu": 0.05},
        {"frac": 0.06,"loc": 1,"mu": 0.05},
        {"frac": 0.06,"loc": 5,"mu": 0.05}
    ],
    100: [
        {"frac": 0.1,"loc": 1,"mu": 0.2},
        {"frac": 0.1,"loc": 5,"mu": 0.05},
        #{"frac": 0.3,"loc": 1,"mu": 0.05},
        #{"frac": 0.3,"loc": 5,"mu": 0.05}
    ]
}
...
early_cutoff = 0.005
out_dir = f"fedprox_{hidden_size}_{dname}_res"
...
def train_model_federated(num_users, part_ratio, device, model, local_model, feature_extractor, criterion, mu, diric, learning_rate, num_epochs=25, local_epochs=1):
    ...
    tracker.write(f"{out_dir}/{dname}_fedprox_dir{diric}_user{num_users}_lr{learning_rate}_mu{mu}_frac{part_ratio}_localit{local_epochs}")
